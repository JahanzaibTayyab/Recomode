...
const SliderScreen = ({navigation}) => {
  ...
  const handleSubmit = async (input) => {
    setShowLoader(true);
    localStorage
      .getKeyFromUserDefaults(constants.KEY_USER_ID)
      .then((userInfo) => {
  ...
  const OnBoardingScreen2ViewCOntroller = () => {
    return (
      <View style={styles.onBoargingViewsContainers}>
        <OnBoard2 ref={onB2Ref} navigation={navigation} />
      </View>
    );
  };
  const OnBoardingScreen3ViewCOntroller = () => {
    return (
      <View style={styles.onBoargingViewsContainers}>
        <OnBoard3 ref={onB3Ref} navigation={navigation} />
      </View>
    );
  };
  const OnBoardingScreen4ViewCOntroller = () => {
    return (
      <View style={styles.onBoargingViewsContainers}>
        <OnBoard4 ref={onB4Ref} navigation={navigation} />
      </View>
    );
  };
    ...
    <>
      ...
      <View style={styles.onBoargingViewsContainers}>
        {showLoader ? (
          ...
          <SwipeGesture
            gestureStyle={styles.swipeGestureContainer}
            ...
            {showBottomView ? (
              <View style={styles.pageControll}>
                <TouchableOpacity
                  ...
                  onPress={() => handleNextBtnPresses()}>
                  {screen === 3 ? (
                    <Text style={styles.textButtonMain}> Place Order </Text>
                  ) : (
